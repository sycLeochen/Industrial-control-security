...
def render_boxplot_figure(df: pd.DataFrame, out_path: str):
    ...
    for ax, (k, title) in zip(axes, metric_specs):
        ...
        ax.boxplot(
            ...
            widths=0.35,
            patch_artist=True,
            boxprops=dict(facecolor="#5b9bd0", alpha=0.85), # 改成藍色區隔
...
def main():
    ...
    model_root = os.path.join(_script_dir, "Model storage space", "OCSVM")
    if not os.path.exists(model_root):
        print("❌ 找不到模型目錄！")
        return
    model_dirs = [d for d in os.listdir(model_root) if d.startswith("Model ")]
    if not model_dirs:
        print("❌ 尚無任何已儲存的 OCSVM 模型！")
        return
    ...
    with open(os.path.join(model_path, "ocsvm_model.pkl"), "rb") as f:
        model = pickle.load(f)
    print("✅ 載入成功！開始評估測試集...\n")
    ...
    table_path = os.path.join(out_dir, "ocsvm_metrics_table.png")
    render_table_figure(dfm, table_path)
    print(f"✅ saved: {table_path}")

    boxplot_path = os.path.join(out_dir, "ocsvm_metrics_boxplot.png")
